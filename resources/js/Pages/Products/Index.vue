<template>
    <div>
        ...
        <div class="flex items-center justify-between mb-6">
            ...
            <Link class="btn-indigo" :href="route('products.create')">
                <span>New</span>
                ...
                <tr v-for="organization in organizations.data" :key="organization.id"
                    ...
                    <td class="border-t">
                        <Link class="flex items-center px-6 py-4 focus:text-indigo-500"
                            ...
                            {{ organization.name }}
                            <icon v-if="organization.deleted_at" name="trash"
                                  class="flex-shrink-0 ml-2 w-3 h-3 fill-gray-400"/>
                        </Link>
                    </td>
                    <td class="border-t">
                        <Link class="flex items-center px-6 py-4" :href="route('organizations.edit', organization.id)"
                              tabindex="-1">
                            {{ organization.city }}
                        </Link>
                    </td>
                    ...
                    <td class="w-px border-t">
                        <Link class="flex items-center px-4" :href="route('organizations.edit', organization.id)"
                              tabindex="-1">
